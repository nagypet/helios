...
static xMbPollContext ctx = {
  // Paramètres
    ...
    .sTcpPort = (char*)DEFAULT_TCP_PORT,
    .sDevice = NULL,
    .xRtu = {
    ...
    .iRtuBaudrate = 0,
    .eRtuDatabits = SERIAL_DATABIT_UNKNOWN,
    .eRtuStopbits = SERIAL_STOPBIT_UNKNOWN,
    .eRtuParity = SERIAL_PARITY_UNKNOWN,
    .bIsVerbose = false,
    .bIsPolling = true,
...
int main2 (int argc, char **argv) {
  ...
  do  {
    ...
    switch (iNextOption) {
      ...
      case 'm':
        ctx.eMode = (eModes)iGetEnum (sModeStr, optarg, sModeList,
      ...
      case 't':
        ctx.eFunction = (eFunctions)iGetInt (sFunctionStr, optarg, 0);
        vCheckEnum (sFunctionStr, ctx.eFunction,
                    iFunctionList, SIZEOF_ILIST (iFunctionList));
        p = index (optarg, ':');
        if (p) {
          ctx.eFormat = (eFormats)iGetEnum (sFormatStr, p + 1, sFormatList, iFormatList,
      ...
      case 'b':
        ...
        break;
      case 'd':
        ctx.xRtu.dbits = (eSerialDataBits)iGetEnum (sRtuDatabitsStr, optarg, sDatabitsList,
                                   iDatabitsList, SIZEOF_ILIST (iDatabitsList));
        break;
      case 's':
        ctx.xRtu.sbits = (eSerialStopBits)iGetEnum (sRtuStopbitsStr, optarg, sStopbitsList,
                                   iStopbitsList, SIZEOF_ILIST (iStopbitsList));
        break;
      case 'P':
        ctx.xRtu.parity = (eSerialParity)iGetEnum (sRtuParityStr, optarg, sParityList,
...
// -----------------------------------------------------------------------------
        ...
        switch (ctx.eFunction) {
          ...
          case eFuncHoldingReg:
            if (ctx.eFormat == eFormatInt) {
              ...
              PDEBUG ("Int[%d]=%d", i, lSwapLong (DINT32 (ctx.pvData, i)));
            }
  ...
  if (ctx.iSlaveCount == -1) {
    ...
    ctx.piSlaveAddr = (int*)malloc (sizeof (int));
...
iGetIntList (const char * name, const char * sList, int * iLen) {
  ...
  if (iCount > 0) {
    ...
    // Allocation
    iList = (int*)calloc (iCount, sizeof (int));
